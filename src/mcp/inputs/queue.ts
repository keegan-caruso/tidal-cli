...
export const getQueueToolSchema = {
  queueId: z
    ...
    .optional()
    .describe(
      'Queue ID to retrieve. If not provided, returns the active queue (or creates one).',
    ),
};
...
export const addToQueueToolSchema = {
  queueId: z
    ...
    .optional()
    .describe(
      'Queue ID to add tracks to. If not provided, uses the active queue.',
    ),
  trackIds: z
    .array(z.string())
    .min(1)
  ...
  position: z
    ...
    .optional()
    .describe(
      'Where to add tracks: "next" (play next) or "last" (end of queue). Default: "last"',
    ),
};
...
export const removeFromQueueToolSchema = {
  queueId: z
    ...
    .optional()
    .describe(
      'Queue ID to remove track from. If not provided, uses the active queue.',
    ),
  trackId: z.string().describe('Track ID to remove'),
  itemId: z
    .string()
    .describe(
      'Item ID of the specific queue entry to remove (from the queue response)',
    ),
};
...
export const skipToTrackToolSchema = {
  ...
  trackId: z.string().describe('Track ID to skip to'),
  itemId: z.string().describe('Item ID of the specific queue entry to skip to'),
};
...
export const updateQueueToolSchema = {
  ...
  repeat: z
    ...
    .optional()
    .describe(
      'Repeat mode: "none" (off), "one" (repeat current track), "all" (repeat all)',
    ),
  shuffled: z.boolean().optional().describe('Enable or disable shuffle mode'),
};
