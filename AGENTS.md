...
in the compiled `dist/` output automatically. At dev/test time, Node 26's `--strip-types`
...
Use pnpm for dependency management. The package manager is pinned in `package.json`.

```bash
pnpm install            # Install dependencies
pnpm run typecheck      # Type-check with tsc (no output files)
pnpm test               # Run tests with Node.js built-in test runner
pnpm run build          # Compile to dist/ with tsc
pnpm run dev:cli        # Run CLI directly from TypeScript (no build needed)
pnpm run dev:mcp        # Run MCP server directly from TypeScript
```

## Node.js TypeScript Execution

The `dev:*` and `test` scripts use Node 26's built-in `--strip-types` flag, which strips
